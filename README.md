...
- 首页：大标题 + “点击进入”
- 身份选择：管理员认证 / 游客访问
- 主界面：上传区（提供 idea / idea 预览 / 点击上传）+ 三个栏目
  - **提供 idea**：填写姓名和 50 字以内的内容
  - **idea 预览**：查看所有 idea（从 GitHub 同步），管理员可审批，通过后由灰色变为实色
  - **点击上传**：上传章节
  - **已有章节**：前 4 章，点击可阅读正文
...
- idea 数据存放在仓库的 `data/ideas.json`，所有浏览器都从这里读写，跨设备自动同步。
- 第一次提交或审批 idea 时，需要粘贴一个 GitHub 个人访问令牌（classic token，勾选 `public_repo`）；令牌只保存在当前浏览器本地，不会上传到仓库或代码里。
- 注意：仓库是公开的，idea 内容公开可见；每次提交/审批会生成一条 commit。
